...
export default function MyTasks() {
  ...
  const priorityColors = {
    ...
    medium: 'bg-blue-100 text-blue-750 dark:bg-blue-950/20 dark:text-blue-400',
    high: 'bg-amber-100 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400',
    urgent: 'bg-rose-100 text-rose-700 dark:bg-rose-950/20 dark:text-rose-455'
  };
  ...
  const handleQuickCommentSubmit = (e) => {
    e.preventDefault();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">My Workspace</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Personal assignments queue and quick actions dashboard</p>
        </div>

        {/* View mode toggle */}
        <div className="flex items-center gap-1.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-1 rounded-xl shadow-soft">
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Total Assigned */}
        <div className="p-4 rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-soft flex items-center gap-3">
          <div className="p-2.5 rounded-lg bg-blue-500/10 text-blue-500">
            <UserCheck size={18} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Assigned Tasks</span>
            <span className="text-lg font-black text-slate-800 dark:text-slate-100">{totalAssigned}</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-soft flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Due Today</span>
            <span className="text-lg font-black text-slate-800 dark:text-slate-100">{dueToday}</span>
          </div>
        </div>

        {/* Overdue */}
        <div className="p-4 rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-soft flex items-center gap-3">
          <div className="p-2.5 rounded-lg bg-red-500/10 text-red-550">
            <AlertTriangle size={18} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Overdue Items</span>
            <span className="text-lg font-black text-slate-800 dark:text-slate-100">{overdue}</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-soft flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-lg font-black text-slate-800 dark:text-slate-100">{completed}</span>
          </div>
        </div>
      </div>
      ...
      {/* Main View Area */}
      {totalAssigned === 0 ? (
        <div className="p-16 rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-soft text-center flex flex-col items-center justify-center">
          ...
          <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">No Tasks Assigned</h3>
          <p className="text-xs text-slate-400 mt-1">Excellent! You are all caught up. No assignments are pending on your queue.</p>
        </div>
      ) : viewMode === 'list' ? (
            ...
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-900 text-xs">
                {myAssignedTasks.map((task) => {
                  ...
                  return (
                    <tr key={task.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                      ...
                      <td className="py-3.5 px-3 font-medium text-slate-655 dark:text-slate-400">
                      ...
                      <td className="py-3.5 pr-6 pl-2 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            onClick={() => navigateToDetails(task.id)}
                            title="View details"
                            className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-905 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 cursor-pointer"
                          >
                            <Eye size={12} />
                          </button>
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="flex items-center gap-0.5 px-2 py-1.5 border border-slate-200 dark:border-slate-800 rounded-lg text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-905 cursor-pointer"
                            >
                              Status
                              <ChevronDown size={10} />
                            </button>
                            {activeDropdownId === task.id && (
                              <>
                                ...
                                <div className="absolute right-0 top-full mt-1 w-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-lg z-20 py-1 text-left">
                                  {['pending', 'inProgress', 'onHold'].map(s => (
                                    <button
                                      key={s}
                                      onClick={() => {
                                      ...
                                      className="w-full text-left px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-850 text-slate-700 dark:text-slate-350 capitalize font-medium cursor-pointer"
                                    >
                          ...
                          {task.status !== 'completed' && (
                            <button
                              ...
                              className="p-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg cursor-pointer transition-colors shadow-sm shadow-emerald-500/10"
                            >
                              <Check size={12} />
                            </button>
                          )}
        ...
        /* Kanban View (Filtered strictly to currentUser tasks) */
          ...
          {columns.map((col) => {
            ...
            return (
              <div
                ...
                className={`flex flex-col rounded-2xl border border-slate-200 dark:border-slate-800 min-h-[400px] overflow-hidden ${col.color} border-t-4 transition-colors`}
              >
                <div className="flex items-center justify-between p-3.5 border-b border-slate-100 dark:border-slate-900 bg-white/60 dark:bg-slate-950/60 backdrop-blur-md">
                  <div className="flex items-center gap-2">
                    <ColIcon size={14} />
                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-350">{col.title}</span>
                  </div>
                ...
                <div className="flex-1 p-3 space-y-2.5 overflow-y-auto">
                  {colTasks.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center border border-dashed border-slate-200 dark:border-slate-800/80 rounded-xl">
                      <span className="text-[9px] font-bold text-slate-450 dark:text-slate-550">Empty Column</span>
                    </div>
                  ) : (
                    colTasks.map((task) => (
                      <div
                        ...
                        onClick={() => navigateToDetails(task.id)}
                        className="group relative bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-blue-500/40 rounded-xl p-3.5 shadow-soft cursor-grab active:cursor-grabbing hover:shadow-soft-lg transition-all"
                      >
                        <div className="space-y-2">
                          <span className="text-[8px] font-mono font-bold text-slate-400 block">{task.id}</span>
                          <h4 className="text-xs font-bold text-slate-850 dark:text-slate-200 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                            {task.title}
                          </h4>
                          
                          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-900 mt-2.5">
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-sm p-5 shadow-soft-lg animate-fade-in">
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-100 dark:border-slate-900 mb-3.5">
              <h3 className="text-xs font-bold text-slate-800 dark:text-slate-100">Add Quick Comment</h3>
              <button
            ...
            <form onSubmit={handleQuickCommentSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-extrabold uppercase text-slate-405 dark:text-slate-500">Comment Content</label>
                <textarea
                  value={quickCommentText}
                  onChange={(e) => setQuickCommentText(e.target.value)}
                  placeholder="Share updates, issues or status summaries..."
                  rows={4}
                  className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500/25 text-slate-800 dark:text-slate-205"
                  required
                  autoFocus
                />
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-900 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setCommentingTaskId(null)}
                  className="px-3.5 py-2 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-655 dark:text-slate-355 rounded-xl text-xs font-bold cursor-pointer"
                >
                  Cancel
                </button>
                <button
